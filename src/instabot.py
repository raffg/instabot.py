...
class InstaBot:
    ...
    def new_auto_mod_follow(self):
        if time.time() > self.next_iteration["Follow"] and \
            ...
            """
            try:
                self.follows_db_c.execute("INSERT INTO usernames (username) VALUES(?)", (self.media_by_tag[0]['node']["owner"]["id"],))
            except:
                self.write_log("I tried to follow this guy once: %s" % (self.media_by_tag[0]['node']["owner"]["id"]))
